...
echo -e "${CYAN}=== Video Analyzer 로컬 테스트 ===${NC}"
echo ""

# 1. Docker 이미지 확인
echo -e "${YELLOW}[1/4] Docker 이미지 확인...${NC}"
if docker images | grep -q "video-analyzer"; then
    echo -e "${GREEN}✅ Docker 이미지 존재${NC}"
else
    echo -e "${RED}❌ Docker 이미지를 찾을 수 없습니다. 먼저 빌드하세요:${NC}"
    echo -e "${CYAN}docker build -t video-analyzer:latest .${NC}"
    exit 1
fi
echo ""
...
docker run -d --name video-analyzer-test \
    -p 8001:8001 \
...
if echo "$HEALTH" | grep -q "healthy"; then
    ...
    docker logs video-analyzer-test
    docker stop video-analyzer-test
    docker rm video-analyzer-test
    exit 1
fi
echo ""
...
DOCS=$(curl -s http://localhost:8001/docs)

if [ -n "$DOCS" ]; then
    echo -e "${GREEN}✅ Swagger UI 접근 가능${NC}"
    echo -e "${CYAN}URL: http://localhost:8001/docs${NC}"
else
...
echo -e "  로그 확인: ${CYAN}docker logs -f video-analyzer-test${NC}"
echo -e "  중지: ${CYAN}docker stop video-analyzer-test${NC}"
echo -e "  삭제: ${CYAN}docker rm video-analyzer-test${NC}"
echo -e "  재시작: ${CYAN}docker restart video-analyzer-test${NC}"
